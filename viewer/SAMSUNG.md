...
On desktop Chrome open `chrome://inspect/#devices`, inspect the Samsung tab, then copy `JSON.stringify(__omni.benchmark)`. Other diagnostics: `__omni.xrPresenting`, `__omni.tracking`, `__omni.resets`, `__omni.xrFramebuffer`, `__omni.errors`. Send Dev A the largest tested count maintaining at least 30 fps, with dimensions/scale and thermal notes. The first 200k run is approximately 30 fps; higher caps are still pending, so this is not yet the chosen production budget. Thermal state and screen-recording status were not recorded.

After the 200k run the agent invoked the page's Exit AR button and verified `xrPresenting=false`, visible pre-session card, hidden HUD, enabled Enter AR button, and no viewer errors. The next prepared URL is the 400k run.
