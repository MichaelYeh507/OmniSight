...
State on 2026-09-20: B0-B8 are on main and deployed. Verified by Dev B on the S21 Ultra (SM-G998U1, Chrome 147): live camera feed, stable anchoring, nudges/yaw/x-ray/saved offsets, Exit and re-entry, ghosts and the portal on the `fake` scene, and the full point-budget table below (every cap up to 774k points holds the 30 fps camera cap). Not yet on the phone: the Enter AR retry, the portal reach clamp and the normal-blended ghosts (all merged or pending since those runs), the `__omni.firstPose` world-frame test, AR from the deployed Pages URL, the heat soak and the A54 (skipped by decision). Real-room alignment (B5) waits for a take that starts at a jig outside the wall; take 012 starts inside the room. Use `npm run phone` for every readout.
