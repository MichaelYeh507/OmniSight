...
| B2 | scene loader, GPU reveal + staleness shader, replay clock, HUD, commander mode | done, `main` 001e2bf. Tests and build pass. **Rendering in a browser not yet confirmed by a human**; do that first |
| B3 | AR mode on the Samsung: WebXR session, `local` space, HUD as dom-overlay, fps budget | next |
| B4 | alignment nudges (x, y, z 1 cm, yaw 0.5 deg), alignment vs x-ray mode, localStorage + URL override | |
...
Planned files: `src/modes/ar.js` (B3), `src/alignment.js` (B4), `src/ghosts.js` (B6), `src/portal.js` and `src/responder.js` (B7).
