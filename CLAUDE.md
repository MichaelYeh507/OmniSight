...
Viewer URL parameters: `mode=ar|commander`, `scene=<folder>`, `t=<s>`, `speed=<x>`, `budget=<max points>`, `psize=<size multiplier>`, `round=1`, `portal=0`, `fbscale=<xr framebuffer scale>`, `ax/ay/az/ayaw` (alignment override). `window.__omni` exposes clock, fps, drawn points, manifest and errors for debugging (Chrome `chrome://inspect#devices` for the Samsung).
